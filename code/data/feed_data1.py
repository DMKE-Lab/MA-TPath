...
class RelationEntityBatcher():
    def __init__(self, input_dir, batch_size, entity_vocab, relation_vocab, tim_vocab, mode = "train"):
        ...
        if self.mode == 'train':
            with open(input_file) as raw_input_file:
                ...
                for line in csv_file:
                    ...
                    if(line[3] in self.tim_vocab):
                        tim = self.tim_vocab[line[3]]
                        self.store.append([e1,r,e2,tim])
                        self.store_all_correct[(e1, r, tim)].add(e2)
                    else:
                        self.store.append([e1,r,e2,0])
                        self.store_all_correct[(e1, r, 0)].add(e2)
    ...
    def yield_next_batch_train(self):
        while True:
            ...
            for i in range(e1.shape[0]):
                all_e2s.append(self.store_all_correct[(e1[i], r[i]), tim[i]])
            assert e1.shape[0] == e2.shape[0] == r.shape[0] == tim.shape[0] == len(all_e2s)
            yield e1, r,tim, e2,  all_e2s
